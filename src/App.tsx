...
import { saveProject, loadProject, listProjects, getRecent, getFavorites, getAutosaveInterval, setAutosaveInterval } from "./projects";
import ProjectsModal from "./ProjectsModal";
import LibraryModal from "./LibraryModal";
import HelpModal from "./HelpModal";
import { toVCD, generateTestbench, waveRows } from "./exportvhd";
import { highlightVhdl } from "./highlight";
...
function Flow() {
  const { t } = useI18n();
  // restaurar el circuito guardado EN EL INICIALIZADOR (sin carrera con el autosave)
  const initData = useRef<{ nodes: RFNode[]; edges: RFEdge[]; entName: string; libCount: number; restored: boolean } | null>(null);
  if (!initData.current) {
    const libCount = loadLibrary(); // la librería antes de usar getDef en el render
  ...
  const { theme, setTheme, fontSize, setFontSize, highContrast, setHighContrast } = useUIState();
  ...
  const restoreFromHash = useCallback((hash: string) => {
    try {
      ...
      setStatus(t("app.status.url_error"));
    }
  }, []);

  /* restaurar desde URL al inicio */
  useEffect(() => {
    const hash = window.location.hash.replace(/^#/, "");
    if (hash) restoreFromHash(hash);
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  /* analytics + tour primera vez */
  useEffect(() => {
    initAnalytics();
    trackEvent("page_view", "Simulador Lógico");
  ...
  /* autosave con debounce (no guarda hasta que la restauración inicial se ha simulado) */
  const saveTimer = useRef<number | undefined>(undefined);
  const armedSave = useRef(false);
  useEffect(() => {
    if (!armedSave.current) {
      armedSave.current = true;
      return;
    } // evita pisar lo guardado en el primer render
    setHasUnsaved(true);
  ...
  /* simulación inicial + estado (el circuito ya se restauró en el inicializador) */
  useEffect(() => {
    const { restored, libCount, nodes: ns } = initData.current!;
    if (ns.length) {
      queueMicrotask(() => commit(nodesRef.current));
      setCustomVersion((v) => v + 1);
    }
    setStatus(
      restored ? t("app.status.restored", { n: ns.length, lib: libCount }) : libCount ? t("app.status.library", { n: libCount }) : t("app.status.empty"),
    );
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const commit = useCallback((ns: RFNode[]) => {
    ...
    // diagnósticos: bucles combinacionales + entradas sin conectar
  ...
  // auto-sync diagrama -> código (solo si está activo). Se llama tras cambios estructurales.
  ...
  // historial para deshacer/rehacer (guarda el estado ANTES de un cambio estructural)
  ...
  // modo retardo: avanza UNA capa de propagación y registra la muestra (glitches visibles)
  ...
  const tick = useCallback(() => {
    ...
    const toggled = nodesRef.current.map((nd) => {
      ...
      const div = (nd.data.div as number) || 1; // multi-reloj: cada reloj a su frecuencia
      if (tickCount.current % div !== 0) return nd;
  ...
  const onNodeClick = useCallback(
    (_: unknown, node: RFNode) => {
      ...
      if (delayModeRef.current) {
        // en modo retardo: aplica el valor sin propagar; el usuario pulsa Δ para ver la propagación
        nodesRef.current = next;
        setNodes(next);
        recordSample(next);
      } else {
  ...
  /* ---------- añadir nodos (drop desde paleta) ---------- */
  const addNodeAt = useCallback(
    (kind: string, pos: { x: number; y: number }) => {
      ...
      if (kind === "REG") {
        data.width = REG_BITS;
        data.bits = new Array(REG_BITS).fill(0) as Sig[];
      }
      if (isBus(kind)) {
        data.width = 4;
        if (kind === "BUSIN" || kind === "BUSOUT") data.bits = new Array(4).fill(0) as Sig[];
      }
      if (isSchematic(kind)) data.inner = instantiateInner(kind); // estado interno propio
  ...
  // cambiar el ancho de un registro (L4): reasigna bits y reubica el cable de CLK
  const setRegWidth = useCallback(
    (id: string, w: number) => {
      pushHistory();
      ...
      const nextEdges = edgesRef.current
        .filter((e) => {
          if (e.target === id) {
            const k = Number((e.targetHandle || "i0").slice(1));
            if (k !== oldW && k >= w) return false;
          }
          if (e.source === id) {
            const k = Number((e.sourceHandle || "o0").slice(1));
            if (k >= w) return false;
          }
          return true;
        })
  ...
  // editar propiedades de un nodo (divisor de reloj, valor inicial, …)
  ...
  const buildVhdl = useCallback(
    (text: string, verbose: boolean) => {
      try {
        pushHistory();
        const res = buildFromVhdl(text);
        idRef.current = res.nodes.length + 1000;
  ...
  /* copiar / pegar / duplicar nodos seleccionados */
  ...
  const importVhd = (file: File) => {
    ...
    r.onload = () => {
      setEditorOpen(true);
      setVhdl(r.result as string);
      buildVhdl(r.result as string, true);
    };
    r.readAsText(file);
  };
  // el proyecto guarda el circuito + la librería de componentes que usa
  const serialize = () =>
    JSON.stringify(
      {
        nodes: nodesRef.current,
        edges: edgesRef.current,
        entName,
        components: serializeLibrary(),
      },
      null,
      2,
    );
  // aplica un proyecto/circuito ya parseado (desde archivo o desde localStorage)
  const applyProject = (data: any) => {
    if (data.components) applyLibrary(data.components);
    else if (data.custom)
      applyLibrary(Object.values(data.custom).map((d: any) => ({ kind: "", label: d.label, mode: "expr", inputs: d.inputs, exprs: d.exprs })));
    saveLibrary();
    setCustomVersion((v) => v + 1);
    edgesRef.current = data.edges || [];
    nodesRef.current = data.nodes || [];
  ...
  const loadJson = (file: File) => {
    ...
    r.onload = () => {
      try {
        applyProject(JSON.parse(r.result as string));
        setStatus(t("app.status.loaded"));
      } catch (e) {
        setStatus(t("app.status.load_error", { msg: (e as Error).message }));
      }
    };
    r.readAsText(file);
  };

  /* ---------- proyectos con nombre (sin descargar) ---------- */
  const saveProjectAs = (name: string) => {
    try {
      saveProject(name, JSON.parse(serialize()));
      setProjName(name);
      setStatus(t("app.status.project_saved", { name }));
    } catch (e) {
      setStatus(t("app.status.project_save_error", { msg: (e as Error).message }));
    }
  };
  const openProject = (name: string) => {
    try {
      applyProject(loadProject(name));
      setProjName(name);
      setStatus(t("app.status.project_opened", { name }));
    } catch (e) {
      setStatus(t("app.status.project_open_error", { msg: (e as Error).message }));
    }
  };
  ...
  const backupExport = () => {
    const backup = {
      kind: "simlog-backup",
      version: 2,
      circuit: { nodes: nodesRef.current, edges: edgesRef.current, entName: entNameRef.current },
  ...
  const backupImport = (file: File) => {
    ...
    r.onload = () => {
      try {
        ...
        setStatus(t("app.status.backup_import_error", { msg: (e as Error).message }));
      }
    };
    r.readAsText(file);
  };
  ...
  const deleteCustom = (kind: string) => {
    if (nodesRef.current.some((n) => n.data.kind === kind)) {
      setStatus(t("app.status.custom_in_use"));
      return;
    }
    removeCustom(kind);
    saveLibrary();
  ...
  const importLib = (file: File) => {
    ...
    r.onload = () => {
      try {
        const n = importLibrary(r.result as string);
        setCustomVersion((v) => v + 1);
        setStatus(t("app.status.library_imported", { n }));
      } catch (e) {
        setStatus(t("app.status.library_import_error", { msg: (e as Error).message }));
      }
    };
    r.readAsText(file);
  };
  /* auto-layout: re-ejecuta el layout automático sobre el circuito actual */
  const relayout = useCallback(() => {
    pushHistory();
    autoLayout(nodesRef.current, edgesRef.current);
    setNodes([...nodesRef.current]);
    setStatus(t("app.status.auto_layout"));
  }, [pushHistory]);

  const reset = () => {
    tickCount.current = 0;
    const next = nodesRef.current.map((nd) => {
      const iv: Sig = nd.data.init ?? 0;
      return { ...nd, data: { ...nd.data, q: iv, prevClk: 0 as Sig, bits: nd.data.bits ? nd.data.bits.map(() => iv) : undefined } };
    });
    commit(next);
    setWaveHist([]);
    setStatus(t("app.status.reset"));
  };
  const clearAll = () => {
  ...
  // exportaciones (§3): VCD del waveform y testbench VHDL
      ...
      <div className="app">
        <header className="topbar">
          ...
          <div className="ctrls">
            <button
              ...
              aria-label={running ? t("app.aria.run_stop") : t("app.aria.run_start")}
            >
              <FiPlay size={14} /> {running ? t("app.stop") : t("app.run")}
            </button>
            <button className="btn" onClick={tick} title={t("app.step")} aria-label={t("app.aria.step")}>
              <FiSkipForward size={14} />
            </button>
            {delayMode && (
              <button className="btn" onClick={deltaTick} title={t("app.delay_propagate")} aria-label={t("app.aria.propagate")}>
                <FiZap size={14} />
              </button>
            )}
            <button className="btn" onClick={reset} title={t("app.reset")} aria-label={t("app.aria.reset")}>
              <FiRefreshCw size={14} />
            </button>
            <button
              className={"btn" + (delayMode ? " btn-primary" : "")}
              title={t("app.delay_mode")}
              onClick={() => {
                const nx = !delayMode;
                setDelayMode(nx);
                delayModeRef.current = nx;
                if (!nx) runSim();
                setStatus(nx ? t("app.status.delay_on") : t("app.status.delay_off"));
              }}
            >
              <FiZap size={14} />
            </button>
          ...
          <div className="ctrls right">
            <HeaderDropdown
              ...
              items={[
                ...
                {
                  label: t("menu.project.export_json"),
                  icon: FiDownload,
                  onClick: () => download((projName || entName || "circuito") + ".json", serialize(), "application/json"),
                },
                { label: t("menu.project.import_vhdl"), icon: FiUpload, onClick: () => fileVhd.current?.click() },
                {
                  label: t("menu.project.export_vhdl"),
                  icon: FiDownload,
                  onClick: () => download((entName || "circuito") + ".vhd", generateVhdl(nodesRef.current, edgesRef.current, entName)),
                },
                { separator: true },
            ...
            <HeaderDropdown
              ...
              items={[
                ...
                {
                  label: theme === "dark" ? t("menu.view.theme_light") : t("menu.view.theme_dark"),
                  icon: theme === "dark" ? FiSun : FiMoon,
                  onClick: () => setTheme((t) => (t === "dark" ? "light" : "dark")),
                },
                { label: t("menu.view.help"), icon: FiHelpCircle, onClick: () => setHelpOpen(true) },
                {
                  label: t("menu.view.share"),
                  icon: FiShare2,
                  onClick: () => {
                    const s = serializeToHash();
                    if (!s) {
                      setStatus(t("app.status.url_serialize_error"));
                      return;
                    }
                    const url = `${window.location.origin}${window.location.pathname}#${s}`;
                    navigator.clipboard
                      .writeText(url)
                      .then(() => setStatus(t("app.status.url_copy")))
                      .catch(() => setStatus(t("app.status.url_copy_error", { url })));
                  },
                },
                ...
                {
                  label: t("menu.view.font", { n: fontSize }),
                  onClick: () => {
                    const sizes = [10, 11, 12, 13, 14, 15, 16, 18, 20];
                    const i = sizes.indexOf(fontSize);
                    setFontSize(sizes[(i + 1) % sizes.length]);
                  },
                },
                {
                  label: t("menu.view.autosave", { n: Math.round(autoSaveMs / 1000) }),
                  onClick: () => {
                    const intervals = [300, 1000, 2000, 5000, 10000, 30000];
                    const i = intervals.indexOf(autoSaveMs);
        ...
        <div className="layout">
          ...
          <section className="center">
            <div className="canvas-wrap" ref={dropRef as any}>
              ...
              >
                <Background gap={20} color="#111113" />
                <Controls />
                <MiniMap pannable zoomable nodeColor="#1c1c1f" maskColor="rgba(0,0,0,.7)" />
              </ReactFlow>
              <div className="status" ref={statusRef} role="status" aria-live="polite" aria-atomic="true">
                {status}
              </div>
          ...
          {editorOpen && (
            <aside className="vhdl-panel">
              <div className="vhdl-head">
                <h2>{t("vhdl.editor")}</h2>
                <button className="btn btn-small" onClick={() => setEditorOpen(false)}>
                  ✕
                </button>
              </div>
              <label className="ent-name">
                {t("vhdl.entity")}
                <input
                  value={entName}
                  onChange={(e) => {
                    setEntName(e.target.value);
                    entNameRef.current = e.target.value;
                    syncCodeFromDiagram();
                  }}
                />
              </label>
              <div className="vhdl-actions">
                <button className="btn btn-small btn-primary" onClick={genVhdl}>
                  {t("vhdl.diagram_to_code")}
                </button>
                <button className="btn btn-small" onClick={() => buildVhdl(vhdl, true)}>
                  {t("vhdl.code_to_diagram")}
                </button>
                <label className="chk" title={t("vhdl.auto_sync_title")}>
                  <input
                    type="checkbox"
                    checked={autoSync}
                    onChange={(e) => {
                      ...
                      if (e.target.checked) setVhdl(generateVhdl(nodesRef.current, edgesRef.current, entNameRef.current));
                    }}
                  />{" "}
                  {t("vhdl.auto_sync")}
                </label>
              ...
              <div className="vhdl-editor">
                <div className="vhdl-gutter" ref={gutterRef} aria-hidden="true">
                  {Array.from({ length: Math.max(1, vhdl.split("\n").length) }, (_, i) => i + 1).join("\n")}
                </div>
                <div className="vhdl-code">
                  ...
                  <textarea
                    ...
                    spellCheck={false}
                    value={vhdl}
                    onChange={(e) => onVhdlChange(e.target.value)}
                    placeholder={t("vhdl.placeholder")}
                    ref={textAreaRef}
                    onScroll={syncScroll}
                  />
                </div>
              ...
              {vhdl.trim() &&
                (lintErrs.length ? (
                  <div className="lint">
                    {lintErrs.map((e, i) => (
                      <div key={i} className="lint-err">
                        ⚠ línea {e.line}: {e.msg}
                      </div>
                    ))}
                  </div>
                ) : (
                  <div className="lint lint-ok">{t("vhdl.lint_ok")}</div>
                ))}
              <div className="vhdl-foot">
                <button className="btn btn-small" onClick={() => navigator.clipboard.writeText(vhdl)}>
                  {t("vhdl.copy")}
                </button>
                <button
                  className="btn btn-small"
                  onClick={() => download((entName || "circuito") + ".vhd", vhdl || generateVhdl(nodesRef.current, edgesRef.current, entName))}
                >
                  {t("vhdl.download")}
                </button>
                <button className="btn btn-small" title={t("vhdl.testbench")} onClick={exportTb}>
                  {t("vhdl.testbench")}
                </button>
        ...
        {libOpen && (
          <LibraryModal
            ...
            inUse={(kind) => nodesRef.current.some((n) => n.data.kind === kind)}
            onExport={(kind) => {
              const d = getDef(kind);
              download(
                d.label + ".simlib.json",
                JSON.stringify(
                  {
                    ...
                    components: [
                      d.schematic
                        ? { kind, label: d.label, mode: "vhdl", vhdlSource: d.vhdlSource }
                        : { kind, label: d.label, mode: "expr", inputs: d.inputs, exprs: d.exprs },
                    ],
                  },
                  null,
                  2,
        ...
        {modalOpen && (
          <CustomModal
            ...
            onFromDiagram={(name) => defineCustomFromDiagram(name, nodesRef.current, edgesRef.current)}
            onCreated={(kind) => {
              saveLibrary();
              setCustomVersion((v) => v + 1);
              setStatus(t("app.status.custom_created", { label: getDef(kind).label }));
            }}
          />
        )}
